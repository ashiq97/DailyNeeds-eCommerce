...
if(isset($_SESSION['user_email']))
{
  ...
  $u_mobile = $resultQ['mobile'];
  $u_address = $resultQ['address'];
...
<div class="col-md-12">
	<div class="row">
	    ...
	    <?php else : ?>
	    	<table class="table table-bordered table-condensed table-striped" id="table1">
	    		<thead >
            <!-- delete -->
            <th >Action</th>
            <!-- delete -->
	    		...
	    		<tbody>
	    			<?php
	    			  foreach ($items as $item) {
	    			  	...
	    			  	<tr>
                  <!-- delete -->
                  <td><button class="btn btn-xs btn-danger" onclick="update_cart('deleteOne','<?php echo $product['id']?>','<?php echo $item['size']?>')"><span class="glyphicon glyphicon-remove"></span></button>
                  </td>
                  <!-- delete -->
	    			  		<td><?php echo $i; ?></td>
	    	...
	    	<br>
	    	<br>
	    	<table class="table table-bordered table-condensed text-right" id="table2">
	    		<legend class="text-center"><b>Totals</b></legend>
...
<a href="<?=((!isset($_SESSION['user_email']))?'/Dailyneeds/admin/rform/user_login.php':'');?>"  data-toggle="<?=((!isset($_SESSION['user_email']))?'':'modal');?>" data-target="<?=((!isset($_SESSION['user_email']))?'':'#checkoutModal');?>" class="btn btn-danger btn-md pull-right" style="margin-left: 250px;">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row">
          ...
          <input type="hidden" name="description" value="<?=$item_count.' item'.(($item_count>1)?'s':'').' from Daily needs.';?>">


            <div id="step1" style="display:block;">
              <div class="form-group col-md-6">
                    <label for="full_name">Full Name :</label>
                    <input type="text" class="form-control" id="full_name" name="full_name" value="<?=$u_name;?>">
              </div>
              <div class="form-group col-md-6">
                <label for="email">Email :</label>
                <input type="email" class="form-control" id="email" name="email" value="<?=$u_email;?>">
              </div>
              <div class="form-group col-md-6">
                <label for="mobile"> Mobile :</label>
                <input type="text" class="form-control" id="mobile" name="mobile" value="<?=$u_mobile;?>">
              </div>
              <!-- <div class="form-group col-md-6">
              ...
              <div class="form-group col-md-6">
                ...
                <textarea style="margin-bottom:10px;" id="city" class="form-control" name="city" rows="6"><?=$u_address;?></textarea>
              </div>
...
function back_address(){
        jQuery('#payment-errors').html(""); // clear any errors
  ...
  var data = {
    'full_name' : jQuery('#full_name').val(),
      ...
      'city'    : jQuery('#city').val(),
          ...
          jQuery.ajax({
             url : '/Dailyneeds/admin/parsers/check_address.php',
             method : 'POST',
             data : data,
